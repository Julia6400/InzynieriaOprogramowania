...
3.  **Utworzenie wirtualnego środowiska (opcjonalnie):**
    ```bash
    python -m venv venv
    ```

4.  **Aktywowanie wirtualnego środowiska (opcjonalnie):**
    * **Windows:**
        ```bash
...
* Merge: Po zakończeniu sprintu/funkcjonalności, gałąź develop była scalona z main.
...
* Julia Grzegorowska: Odpowiedzialna za klasy Patient i Appointment, oraz związane z nimi testy jednostkowe, a także testy integracyjne oraz plik main.py zawierający przykładowe użycie aplikacji.
